  ...
  <div class="well">
    ...
    @if (count($item_types) > 0)
      ...
      <div class="form=group col-xs-3">
        ...
        {{Form::number('item_weight',  $item->item_weight, ['class' => 'form-control', 'placeholder' => '0.00'])}}
      </div>
      <div class="form=group col-xs-3">
        {{Form::label('item_amount', 'Item Cost:')}}
        {{Form::number('item_amount',  $item->item_amount, ['class' => 'form-control', 'placeholder' => '0.00'])}}
      </div>
    </div>
    <div class="row">
      <div class="form=group col-xs-3">
        {{Form::label('item_count', 'Number of Items:')}}
        {{Form::number('item_count',  $item->item_count, ['class' => 'form-control', 'placeholder' => '0.00'])}}
      </div>
      @if (count($uom) > 0)
        <div class="form=group col-xs-3">
